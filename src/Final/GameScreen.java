...
public class GameScreen extends Applet implements MouseListener,
...

{
	...
	public GameScreen() {
		...
		localjpanel.setPreferredSize(new Dimension(1024, 668));
		localjpanel.setBorder(BorderFactory.createEmptyBorder(0, 4, 4, 4));
		add(localjpanel, BorderLayout.SOUTH);
	}

	public void init() {
		String imageString[] = { "S20.png", "S20 (1).png", "S20 (2).png",
				"S20 (3).png", "S20 (4).png", "S20 (5).png", "S30.png",
				"S30 (1).png", "S40.png", "S40 (1).png", "H50.png", "B-10.png" };
	...
	public void loadpic(int i, int k) {
		current = pics[i];
		currentEx = burst[k];
		posX = (int) (Math.random() * 750) + 200; // for random position of
													// balloon
		j = 0;
		while (j < 751 && boom == 2) {
			pause(100);
	...
	public void paint(Graphics g) {
		...
		for (int x = 0; x < current.getHeight(null); x++) { // For bursting of
															// balloon,
			for (int y = 0; y < current.getWidth(null); y++) {
				if (tip == (posX + y) && (548 - j + x) == pos) { // if the
																	// coordinate
																	// matches
		...
		if (boom == 1) {
			...
			g.drawImage(currentEx, posBurstX, posBurstY, this); // drawing image
																// of explosion
			boom = 3;
			...
			// arrow
			g.drawImage(arrow, 0, posY, null); // arrow always at the left of
												// screen
			posAX = i;
			boom = 2;
			if (cursor == 1 && i < 1000) {
